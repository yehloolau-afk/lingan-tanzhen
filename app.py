...
MOCKUP_SPEC = (
    ...
    "═══ 第二部分：核心界面示意 ═══\n"
    "3个手机屏幕横向排列，严格遵循 iPhone 屏幕比例（9:19.5），展示产品核心操作界面。\n\n"
    "【重要】整体容器必须自带横向滚动，不依赖外层容器：\n"
    "整体容器：width:100%; box-sizing:border-box; background:#F5F5F7; padding:28px 0 36px; overflow-x:auto; -webkit-overflow-scrolling:touch;\n"
    "标题行：font-size:11px; font-weight:700; color:#8C8C8C; letter-spacing:1px; text-transform:uppercase;\n"
    "  text-align:center; margin-bottom:20px; padding:0 24px; 内容：KEY SCREENS\n"
    "屏幕行：display:flex; align-items:flex-start; gap:20px; width:max-content; min-width:100%; padding:0 24px; box-sizing:border-box;\n\n"
    "每个手机单元（步骤标签 + 手机外框）：\n"
    "步骤标签：text-align:center; font-size:11px; font-weight:700; color:#8E8E93; letter-spacing:0.5px; margin-bottom:10px;\n"
    "手机外框：width:240px; flex-shrink:0; border-radius:48px; overflow:hidden;\n"
    "  box-shadow:0 24px 64px rgba(0,0,0,0.30),0 4px 16px rgba(0,0,0,0.12); border:8px solid #1C1C1E; background:#1C1C1E;\n\n"
    "屏幕内容区（严格比例，9:19.5）：\n"
    "  background:#F2F2F7; width:224px（240 - 8×2 border）\n"
    "  height:486px（224 × 19.5/9 = 486px，严格固定）\n"
    "  overflow:hidden; display:flex; flex-direction:column;\n\n"
    "每屏层次（高度分配，总计486px）：\n"
    "① 状态栏：height:44px; flex-shrink:0; padding:12px 18px 0; display:flex; justify-content:space-between; align-items:center;\n"
    "   background与导航栏同色; 左='9:41' font-size:14px font-weight:600 color:#000; 右='▶︎ ◈ 🔋' font-size:12px\n"
    "② 导航栏：height:48px; flex-shrink:0; padding:0 18px; display:flex; align-items:center; justify-content:space-between;\n"
    ...
    "③ 内容区：flex:1（自动填充剩余高度 ≈ 394px）；overflow:hidden\n"
    "   卡片间距padding:12px 16px; 卡片font-size:15px; 列表标题font-size:15px; 辅助文字font-size:13px\n"
    "④ 底部主按钮（若有）：height:50px; flex-shrink:0; margin:0 16px 12px; border-radius:12px; font-size:17px font-weight:600\n"
    "   或 Tab Bar：height:50px; flex-shrink:0; border-top:0.5px solid #F0F0F0; font-size:11px\n\n"
    "连接箭头（2个）：align-self:center; padding-bottom:48px; font-size:24px; color:#C7C7CC; flex-shrink:0;\n\n"

    "═══ Ant Design Mobile 组件规范（严格执行） ═══\n"
    "主色：科技/工具→#1677FF；创意→#722ED1；社交→#FA8C16；效率→#52C41A；金融→#096DD9\n\n"
    "卡片：background:#FFF; border-radius:8px; box-shadow:0 1px 4px rgba(0,0,0,0.08); padding:12px 14px; margin:0 12px 8px;\n"
    "列表行：min-height:44px; padding:0 16px; display:flex; align-items:center; background:#FFF;\n"
    "  border-bottom:1px solid #F5F5F5; 文字font-size:15px color:#000; 右侧chevron › color:#C7C7CC font-size:14px\n"
    ...
    "主按钮：display:block; background:主色; color:#FFF; border-radius:12px;\n"
    "  height:50px; line-height:50px; font-size:17px; font-weight:600; text-align:center;\n"
    "  margin:0 16px; letter-spacing:0.5px;\n"
    "次按钮：border:1.5px solid 主色; color:主色; background:#FFF; border-radius:12px;\n"
    "  height:44px; line-height:42px; font-size:15px; text-align:center; margin:0 16px;\n"
    "输入框：background:#F5F5F5; border-radius:8px; border:none; padding:10px 14px; font-size:15px; color:#000;\n"
    "文字层次：大标题20px/700 color:#000; 正文15px/400 color:#000; 辅助13px/400 color:#8C8C8C\n"
    "Tab Bar：height:50px; flex-shrink:0; background:#FFF; border-top:0.5px solid #F0F0F0;\n"
    "  display:flex; 每项：flex:1; display:flex; flex-direction:column; align-items:center; justify-content:center;\n"
    "  emoji font-size:20px; 文字font-size:10px; active color:主色; inactive color:#8C8C8C\n\n"
